...
describe('First Api Tests', () => {
    ...
    it('Consume HEAD Service', async () => {
        ...
        parametersToValidate.every(parameter =>
            expect(headResponse.header[parameter]).to.be.equals(getResponse.header[parameter]));
    });
    it('Consume PATCH Service', async () => {
        ...
        const response = await agent
            ...
            .send(testData);
        expect(response.status).to.equal(statusCode.OK);
        for (let testDataKey in testData) {
            expect(response.body.json[testDataKey]).to.be.equals(testData[testDataKey]);
        }
    });
